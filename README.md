...
│   └── presets.py          # 预定义配置模板
├── data/                    # 数据处理
│   ├── base.py             # DatasetReader 抽象基类
│   ├── models.py           # 数据模型 (Sample, TestCase, Solution)
│   └── codecontests.py     # CodeContests 数据集读取器
├── clients/                 # API 客户端
│   ├── openai_client.py    # OpenAI API 封装
│   └── sandbox_client.py   # 沙箱 API 封装
├── core/                    # 核心逻辑
│   ├── generator.py        # 测试用例生成器
│   └── validator.py        # 解决方案验证器
├── parallel/                # 并行处理
│   ├── api_pool.py         # API 连接池
│   └── processor.py        # 并行处理器
├── prompts/                 # 提示模板
│   └── templates.py        # LLM 提示模板
├── utils/                   # 工具函数
│   ├── helpers.py          # 通用工具
│   └── logger.py           # 日志管理
└── examples/                # 示例
    └── custom_reader.py    # 自定义数据集读取器示例
```
...
# 安装依赖
pip install openai pydantic requests tqdm datasets
```
